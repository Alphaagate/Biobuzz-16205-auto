...
@Configurable
@Autonomous
public class blue2spike21 extends OpMode {
    ...
    private final double SHOOT_TIME = 0.6;
    ...
    public void buildPaths() {
        ...
        driveToSet2 = follower.pathBuilder()
                .addPath(
                        new BezierCurve(
                                new Pose(53.423, 74.143),
                                new Pose(40.847, 52.795),
                                new Pose(18.474, 55.173)
                        )
                )
...
//                    prepareToShoot();
            ...
            case SHOOT_GATE:
                ...
                if (pathTimer.getElapsedTimeSeconds() > SHOOT_TIME) {
                    ...
                    switch (count) {
                        case 1:
                            // After 1st gate shot — e.g. adjust angle, change intake speed
                            follower.followPath(driveToGate, true);
                            setPathState(PathState.DRIVE_TO_GATE);
                            break;

                        case 2:
                            // After 2nd gate shot — e.g. slightly different path or mechanism state
                            follower.followPath(driveToGate, true);
                            setPathState(PathState.DRIVE_TO_GATE);
                            break;

                        case 3:
                            // After 3rd gate shot — e.g. last gate attempt, maybe different position
                            follower.followPath(driveToGate, true);
                            setPathState(PathState.DRIVE_TO_GATE);
                            break;
